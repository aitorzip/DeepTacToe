...
void TCPSocket::connect(void) noexcept(false)
{
    ...
    TEST_ASSERT(_mode != eNone, "Trying to connect on already active socker?");
...
void TCPSocket::listen(uint8_t queueSize) noexcept(false)
{
    ...
    TEST_ASSERT(_mode != eNone, "Trying to listen on already active socker?");
...
TCPSocket TCPSocket::accept(void) noexcept(false)
{
    ...
    // Test if listen() was called
    TEST_ASSERT(_mode != eServer, "Trying to accept on non server socket?");
...
//#endif
        ...
        TEST_ASSERT(true, "Setting reuse port on closed socket?");
    }
}
...
void TCPSocket::reuseAddr(eState val)
{
    ...
    if( _fd[0] != ISocket::FD_INIT )
    {
        ...
        TEST_ASSERT(true, "Setting reuseAddr on closed socket?");
    }
}
...
void TCPSocket::nagle(eState val)
{
    ...
    if( _fd[0] != ISocket::FD_INIT )
    {
        ...
        TEST_ASSERT(true, "Setting nagle on closed socket?");
    }
}
...
void TCPSocket::noneBlockingMode(eState val)
{
    ...
    if( _fd[0] != ISocket::FD_INIT )
        ...
        TEST_ASSERT(true, "Setting none blocking mode on closed socket?");
    }
}
...
int32_t TCPSocket::send( const uint8_t* pData, uint32_t size) const
{
    ...
    TEST_ASSERT( pData == nullptr, "Trying to send nullptr?");
    TEST_ASSERT( size == 0,        "Trying to send enpty message?");
...
int32_t TCPSocket::receive( uint8_t*  data, uint32_t size) const
{
    ...
    TEST_ASSERT( data == nullptr, "Trying to recive with nullptr?");
    TEST_ASSERT( size == 0,       "Trying to recive zero buffer?");
